...
function M.new( frame_builder, ace_serializer, notify )
	...
	local function submit_request()
		...
		for _, item in request_frame.items do
			...
			if total < item.request_count then
				...
				notify.add( string.format( "You have requested %d %s, but only %d is available.", item.request_count, item.name, total ), "Error",
					m.get_server_timestamp(), 2 )
				return
			end

			if item.request_count == 0 then
				m.error( string.format( "You have requested 0 %s", item.name ) )
				notify.add( string.format( "You have requested 0 %s", item.name ), "Error", m.get_server_timestamp(), 2 )
				return
			end
			...
			local function add_request( player, count )
				requests[ player ] = requests[ player ] or {
					from = m.player,
					to = player,
					timestamp = m.get_server_timestamp(),
		...
		for _, request in requests do
			...
			m.notify.add( string.format( "Request sent to %s", request.to ), m.NotificationType.Info, m.get_server_timestamp(), 2 )
		end
		request_frame.clear()
	end
	...
	local function create_slot( parent, slot_index )
		...
		slot_frame:SetScript( "OnEnter", function()
			if this.item then
				...
				GameTooltip:SetHyperlink( string.format( "item:%d", this.item.id ) )
			end
		end )
		...
		slot_frame:SetScript( "OnClick", function()
			if IsShiftKeyDown() and slot_frame.item then
				if ChatFrameEditBox:IsVisible() then
					ChatFrameEditBox:Insert( m.make_item_link( slot_frame.item.id, slot_frame.item.name, slot_frame.item.quality ) )
					return
				end
			end
	...
	local function create_inbox_item( parent )
		...
		local btn_done = m.GuiElements.create_button( frame, "Completed", nil, function()
			...
			m.notify.add( string.format( "Request to %s fulfilled.", frame.request.to ), m.NotificationType.Info, m.get_server_timestamp() )
			table.remove( m.db.requests, frame.index )
			inbox_frame.refresh()
			refresh()
		end, clear_cursor )
